...
(function() {
    ...
    AccelWaiter.addSingle('grab-ticket-number', function() {
        if (checkExist('#iframe-page-container') && getCookie('grab-ticket-number') == 'true') {
    ...
    AccelWaiter.addSingle('trust-account-run', function() {
        ...
        // Step 5 - Grab all Payment Info then go to Fee Page
        if (checkRun('trust-account', ++n) && checkExist('a#save')) {
            if (!checkExist('a.my-click-added')) {
                ...
                addClass(find('a#save'), 'my-click-added');
            }
            counter = 0;
        }
        // Step 6 - If Fee skip a head otherwise go to Add Fee Page
        ...
        // Step 7 - Add Fee Page waiting for info to be filled out
        ...
        // STUCK HERE
        // Step 20 might have been clicking void before the invoice was selected. Need to test again. 
        
        // Step 21 - Void Fee Popup
        ...
        // Step 22
        orderList('trust-account', ++n, 'Invoice #');
        // Step 23 - Invoice the Void
        setRun('trust-account', ++n, 'a#invoice', function() {
            let row = find('tr#row1');
            setField(find('#ac360_list_id', row), 'click', 'true');
        });
        // Step 24 - go to Payment page
        clickPage('trust-account', ++n, '#Payments a');
        // Step 25 - Refund Payment to Trust Accound
        clickPage('trust-account', ++n, 'a#a_Refund');
        // Step 26 - Go to Refund Page
        clickPage('trust-account', ++n, 'a#update');
        // Step 27 - Fill Out Refund Page
